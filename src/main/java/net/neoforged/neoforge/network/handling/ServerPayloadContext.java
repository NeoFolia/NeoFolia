...
public record ServerPayloadContext(ServerCommonPacketListener listener, Identifier payloadId) implements IPayloadContext {
    ...
    public CompletableFuture<Void> enqueueWork(Runnable task) {
        if (listener instanceof ServerPlayerConnection) {
            return this.enqueueGameplayWork(() -> {
                task.run();
                return null;
            });
        }
    ...
    public <T> CompletableFuture<T> enqueueWork(Supplier<T> task) {
        if (listener instanceof ServerPlayerConnection) {
            return this.enqueueGameplayWork(task);
        }
    ...
    private <T> CompletableFuture<T> enqueueGameplayWork(Supplier<T> task) {
        CompletableFuture<T> future = new CompletableFuture<>();
        PacketProcessor processor = listener.getPacketProcessor();
        Runnable packetProcessorTask = () -> {
            ServerPlayer player = ((ServerPlayerConnection) this.listener).getPlayer();
        ...
        try {
            if (processor.isSameThread()) {
                packetProcessorTask.run();
            } else {
                processor.scheduleIfPossible(packetProcessorTask);
            }
        } catch (Throwable throwable) {
            future.completeExceptionally(throwable);
        }

        return NetworkRegistry.guard(future, this.payloadId);
    }
