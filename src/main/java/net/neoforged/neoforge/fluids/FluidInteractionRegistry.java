...
public final class FluidInteractionRegistry {
    ...
    public static boolean canInteract(Level level, BlockPos pos) {
        ...
        for (Direction direction : LiquidBlock.POSSIBLE_FLOW_DIRECTIONS) {
            ...
            List<InteractionInformation> interactions = getInteractions(state.getFluidType());
    ...
    private static synchronized List<InteractionInformation> getInteractions(FluidType source) {
        List<InteractionInformation> interactions = INTERACTIONS.getOrDefault(source, Collections.emptyList());
