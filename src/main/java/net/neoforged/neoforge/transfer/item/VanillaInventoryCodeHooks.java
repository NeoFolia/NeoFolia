...
import net.neoforged.neoforge.transfer.RegionizedBlockResourceHandler;
import net.neoforged.neoforge.transfer.RegionizedEntityResourceHandler;
import net.neoforged.neoforge.transfer.RegionizedPlayerResourceHandler;
...
public class VanillaInventoryCodeHooks {
    ...
    public static boolean extractHook(Hopper dest, ResourceHandler<ItemResource> handler) {
        if (handler instanceof RegionizedBlockResourceHandler<ItemResource> regionizedHandler) {
            if (RegionizedTickContext.isRegionThread() && !regionizedHandler.isOnOwningRegion()) {
                return false;
            }
            return regionizedHandler.callOnBlockRegion("hopper item handler extract", () -> extractHookOnCurrentThread(dest, handler));
        } else if (handler instanceof RegionizedEntityResourceHandler<ItemResource> regionizedHandler) {
            if (RegionizedTickContext.isRegionThread() && !regionizedHandler.isOnOwningRegion()) {
                return false;
            }
            return regionizedHandler.callOnEntityRegion("hopper entity item handler extract", () -> extractHookOnCurrentThread(dest, handler));
        } else if (handler instanceof RegionizedPlayerResourceHandler<ItemResource> regionizedHandler) {
            if (RegionizedTickContext.isRegionThread() && !regionizedHandler.isOnOwningRegion()) {
                return false;
            }
            return regionizedHandler.callOnPlayerRegion("hopper player item handler extract", () -> extractHookOnCurrentThread(dest, handler));
        }

        return extractHookOnCurrentThread(dest, handler);
    }

    private static boolean extractHookOnCurrentThread(Hopper dest, ResourceHandler<ItemResource> handler) {
    ...
    public static boolean insertHook(HopperBlockEntity hopper, ResourceHandler<ItemResource> itemHandler) {
        if (itemHandler instanceof RegionizedBlockResourceHandler<ItemResource> regionizedHandler) {
            if (RegionizedTickContext.isRegionThread() && !regionizedHandler.isOnOwningRegion()) {
                return false;
            }
            return regionizedHandler.callOnBlockRegion("hopper item handler insert", () -> insertHookOnCurrentThread(hopper, itemHandler));
        } else if (itemHandler instanceof RegionizedEntityResourceHandler<ItemResource> regionizedHandler) {
            if (RegionizedTickContext.isRegionThread() && !regionizedHandler.isOnOwningRegion()) {
                return false;
            }
            return regionizedHandler.callOnEntityRegion("hopper entity item handler insert", () -> insertHookOnCurrentThread(hopper, itemHandler));
        } else if (itemHandler instanceof RegionizedPlayerResourceHandler<ItemResource> regionizedHandler) {
            if (RegionizedTickContext.isRegionThread() && !regionizedHandler.isOnOwningRegion()) {
                return false;
            }
            return regionizedHandler.callOnPlayerRegion("hopper player item handler insert", () -> insertHookOnCurrentThread(hopper, itemHandler));
        }

        return insertHookOnCurrentThread(hopper, itemHandler);
    }

    private static boolean insertHookOnCurrentThread(HopperBlockEntity hopper, ResourceHandler<ItemResource> itemHandler) {
    ...
    public static ContainerOrHandler getEntityContainerOrHandler(Level level, double x, double y, double z, @Nullable Direction side) {
        AABB searchBox = new AABB(x - 0.5D, y - 0.5D, z - 0.5D, x + 0.5D, y + 0.5D, z + 0.5D);
        AABB regionBox = RegionizedWorldGuard.restrictToCurrentRegion(level, searchBox);
        if (regionBox == null) {
