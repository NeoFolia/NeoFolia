...
public interface ItemAccess {
    ...
    static ItemAccess forPlayerInteraction(Player player, InteractionHand hand) {
        if (player instanceof ServerPlayer serverPlayer) {
            return createRegionizedPlayerAccess(serverPlayer, "player interaction item access", () -> createPlayerInteractionAccess(player, hand));
        }

        return createPlayerInteractionAccess(player, hand);
    }

    private static ItemAccess createPlayerInteractionAccess(Player player, InteractionHand hand) {
        if (player.hasInfiniteMaterials()) {
            return createInfiniteMaterialsAccess(player, player.getItemInHand(hand));
        } else {
            return createPlayerSlotAccess(player, switch (hand) {
    ...
    static ItemAccess forInfiniteMaterials(Player player, ItemStack contents) {
        if (player instanceof ServerPlayer serverPlayer) {
            return createRegionizedPlayerAccess(serverPlayer, "infinite materials item access", () -> createInfiniteMaterialsAccess(player, contents));
        }

        return createInfiniteMaterialsAccess(player, contents);
    }

    private static ItemAccess createInfiniteMaterialsAccess(Player player, ItemStack contents) {
        if (!player.hasInfiniteMaterials()) {
    ...
    static ItemAccess forPlayerCursor(Player player, AbstractContainerMenu menu) {
        if (player instanceof ServerPlayer serverPlayer) {
            return createRegionizedPlayerAccess(serverPlayer, "player cursor item access", () -> createPlayerCursorAccess(player, menu));
        }

        return createPlayerCursorAccess(player, menu);
    }

    private static ItemAccess createPlayerCursorAccess(Player player, AbstractContainerMenu menu) {
    ...
    static ItemAccess forPlayerSlot(Player player, int slot) {
        if (player instanceof ServerPlayer serverPlayer) {
            return createRegionizedPlayerAccess(serverPlayer, "player slot item access", () -> createPlayerSlotAccess(player, slot));
        }

        return createPlayerSlotAccess(player, slot);
    }

    private static ItemAccess createPlayerSlotAccess(Player player, int slot) {
    ...
    private static ItemAccess createRegionizedPlayerAccess(ServerPlayer player, String reason, Supplier<ItemAccess> supplier) {
        var server = player.level().getServer();
        if (server != null && server.regionizedTaskDispatcher().isRunning()) {
            ItemAccess delegate = RegionizedWorldGuard.callOnPlayerRegionAndWait(player, reason, supplier);
            return new RegionizedPlayerItemAccess(player, delegate);
        }
