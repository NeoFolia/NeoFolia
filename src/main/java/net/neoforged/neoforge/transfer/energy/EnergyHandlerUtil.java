...
public final class EnergyHandlerUtil {
    ...
    public static int move(
        ...
        if (transaction == null) {
            RegionizedEnergyHandler fromRegionized = unwrapRegionized(from);
            RegionizedEnergyHandler toRegionized = unwrapRegionized(to);
            if (fromRegionized != null || toRegionized != null) {
                EnergyHandler fromDelegate = fromRegionized != null ? fromRegionized.delegate() : from;
                EnergyHandler toDelegate = toRegionized != null ? toRegionized.delegate() : to;
                return callOnRegionizedHandlers(
                        "energy handler move",
                        fromRegionized,
                        toRegionized,
                        () -> moveOnCurrentThread(fromDelegate, toDelegate, amount, null));
            }
        }

        return moveOnCurrentThread(from, to, amount, transaction);
    }

    private static int moveOnCurrentThread(
    ...
    @Nullable
    private static RegionizedEnergyHandler unwrapRegionized(EnergyHandler handler) {
        if (handler instanceof RegionizedBlockEnergyHandler regionizedHandler) {
            return new RegionizedEnergyHandler(
                    regionizedHandler.region(),
                    regionizedHandler.level().getServer(),
                    regionizedHandler.delegateForReservedRegion());
        }

        if (handler instanceof RegionizedEntityEnergyHandler regionizedHandler) {
            return new RegionizedEnergyHandler(
                    regionizedHandler.region(),
                    ((net.minecraft.server.level.ServerLevel) regionizedHandler.entity().level()).getServer(),
                    regionizedHandler.delegateForReservedRegion());
        }
    ...
    private static int callOnRegionizedHandlers(
            String reason,
            @Nullable RegionizedEnergyHandler first,
            @Nullable RegionizedEnergyHandler second,
            IntSupplier task) {
        ...
        server.regionizedTaskDispatcher().awaitRegions(reason, regions, primary, () -> result.set(task.getAsInt()));
        return result.get();
    }

    private record RegionizedEnergyHandler(RegionKey region, @Nullable MinecraftServer server, EnergyHandler delegate) {}
