...
public final class EntityCapability<T, C extends @Nullable Object> extends BaseCapability<T, C> {
    ...
    public T getCapability(Entity entity, C context) {
        for (var provider : providers.getOrDefault(entity.getType(), List.of())) {
            ...
            if (ret != null)
                return wrapRegionizedEntityCapability(entity, ret);
        }
        return null;
    }

    @SuppressWarnings({ "unchecked", "rawtypes" })
    private T wrapRegionizedEntityCapability(Entity entity, T value) {
        if (!(entity.level() instanceof ServerLevel)) {
            return value;
        }

        if ((this == Capabilities.Item.ENTITY || this == Capabilities.Item.ENTITY_AUTOMATION || this == Capabilities.Fluid.ENTITY)
                && value instanceof ResourceHandler<?> handler) {
            return (T) RegionizedEntityResourceHandler.of(entity, (ResourceHandler) handler);
        }

        if (this == Capabilities.Energy.ENTITY && value instanceof EnergyHandler handler) {
            return (T) RegionizedEntityEnergyHandler.of(entity, handler);
        }
