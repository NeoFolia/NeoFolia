...
class DimensionsCommand {
    static ArgumentBuilder<CommandSourceStack, ?> register() {
        return Commands.literal("dimensions")
                ...
                .executes(ctx -> {
                    if (CommandUtils.scheduleGlobalIfRegionized(ctx.getSource(), () -> {
                        ...
                        sendDimensions(ctx.getSource(), types);
                    })) {
                        return 0;
                    }
    ...
    private static void sendDimensions(CommandSourceStack source, Map<Identifier, List<Component>> types) {
