...
class TimeSpeedCommand {
    ...
    private static int query(CommandSourceStack source) {
        if (CommandUtils.scheduleGlobalIfRegionized(source, () -> query(source))) {
            return 0;
        }
    ...
    private static int setSpeed(CommandSourceStack source, float speed) {
        if (CommandUtils.scheduleGlobalIfRegionized(source, () -> setSpeed(source, speed))) {
            return 0;
        }
    ...
    private static int setDaylength(CommandSourceStack source, int minutes) {
        if (CommandUtils.scheduleGlobalIfRegionized(source, () -> setDaylength(source, minutes))) {
            return 0;
        }
    ...
    private static int setDefault(CommandSourceStack source) {
        if (CommandUtils.scheduleGlobalIfRegionized(source, () -> setDefault(source))) {
            return 0;
        }
