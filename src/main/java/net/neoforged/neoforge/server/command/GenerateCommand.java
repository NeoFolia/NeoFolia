...
class GenerateCommand {
    ...
    private static int executeGeneration(CommandSourceStack source, BlockPos pos, int chunkRadius, boolean progressBar) {
        if (RegionizedTickContext.isRegionThread()) {
            ...
            if (source.getServer().regionizedTaskDispatcher().tryExecuteGlobal(() -> executeGeneration(source, immutablePos, chunkRadius, progressBar))) {
                return 0;
            }

            throw new IllegalStateException("Cannot start chunk generation from region worker while region dispatcher is not running");
        }
    ...
    private static int stopGeneration(CommandSourceStack source) {
        if (RegionizedTickContext.isRegionThread()) {
            if (source.getServer().regionizedTaskDispatcher().tryExecuteGlobal(() -> stopGeneration(source))) {
                return 0;
            }

            throw new IllegalStateException("Cannot stop chunk generation from region worker while region dispatcher is not running");
        }
    ...
    private static int getGenerationStatus(CommandSourceStack source) {
        if (RegionizedTickContext.isRegionThread()) {
            if (source.getServer().regionizedTaskDispatcher().tryExecuteGlobal(() -> getGenerationStatus(source))) {
                return 0;
            }

            throw new IllegalStateException("Cannot read chunk generation status from region worker while region dispatcher is not running");
        }
    ...
    private static int getGenerationHelp(CommandSourceStack source) {
        if (RegionizedTickContext.isRegionThread()) {
            if (source.getServer().regionizedTaskDispatcher().tryExecuteGlobal(() -> getGenerationHelp(source))) {
                return 0;
            }

            throw new IllegalStateException("Cannot read chunk generation help from region worker while region dispatcher is not running");
        }
