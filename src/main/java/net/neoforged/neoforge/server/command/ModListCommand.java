...
class ModListCommand {
    static ArgumentBuilder<CommandSourceStack, ?> register() {
        return Commands.literal("mods")
                ...
                .executes(ctx -> {
                    if (CommandUtils.scheduleGlobalIfRegionized(ctx.getSource(), () -> sendModList(ctx.getSource()))) {
                        return 0;
                    }

                    sendModList(ctx.getSource());
                    return 0;
                });
    }

    private static void sendModList(CommandSourceStack source) {
        source.sendSuccess(() -> CommandUtils.makeTranslatableWithFallback("commands.neoforge.mods.list",
