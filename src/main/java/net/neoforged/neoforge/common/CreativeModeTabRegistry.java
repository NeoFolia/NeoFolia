...
public final class CreativeModeTabRegistry {
    ...
    private static void runInServerThreadIfPossible(BooleanConsumer runnable) {
        ...
        if (server != null) {
            ...
            if (RegionizedTickContext.isRegionThread() || server.regionizedTaskDispatcher().isRunning()) {
                if (server.regionizedTaskDispatcher().tryExecuteGlobal(() -> runnable.accept(true))) {
                    return;
                }
            }
