...
public class DataMapHooks {
    ...
    /** Used in a gametest */
    @ApiStatus.Internal
    public static volatile boolean didHaveToFallbackToVanillaMaps = false;
    ...
    @Nullable
    @SuppressWarnings("deprecation")
    public static synchronized Block getPreviousOxidizedStage(Block block) {
    ...
    @Nullable
    @SuppressWarnings("deprecation")
    public static synchronized Block getBlockUnwaxed(Block block) {
    ...
    @SubscribeEvent
    static synchronized void onDataMapsUpdated(DataMapsUpdatedEvent event) {
        event.ifRegistry(Registries.BLOCK, registry -> {
            ...
            HoneycombItem.WAX_OFF_BY_BLOCK.get().forEach((after, before) -> {
                if (!INVERSE_WAXABLES_DATAMAP_INTERNAL.containsKey(after)) {
                    INVERSE_WAXABLES_DATAMAP_INTERNAL.put(after, before);
                    didHaveToFallbackToVanillaMaps = true;
                }
            });
        });
