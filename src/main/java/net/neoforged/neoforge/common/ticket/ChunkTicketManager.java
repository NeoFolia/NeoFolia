/*
 * Copyright (c) Forge Development LLC and contributors
 * SPDX-License-Identifier: LGPL-2.1-only
 */

package net.neoforged.neoforge.common.ticket;

import java.util.Collection;
import java.util.Collections;
import java.util.ArrayList;
import java.util.Set;
import java.util.WeakHashMap;
import net.minecraft.world.level.ChunkPos;

public class ChunkTicketManager<T> implements ITicketGetter<T> {
    private final Set<SimpleTicket<T>> tickets = Collections.newSetFromMap(new WeakHashMap<>());
    public final ChunkPos pos;

    public ChunkTicketManager(ChunkPos pos) {
        this.pos = pos;
    }

    @Override
    public synchronized void add(SimpleTicket<T> ticket) {
        this.tickets.add(ticket);
    }

    @Override
    public synchronized void remove(SimpleTicket<T> ticket) {
        this.tickets.remove(ticket);
    }

    @Override
    public synchronized Collection<SimpleTicket<T>> getTickets() {
        return new ArrayList<>(tickets);
    }
}
