...
public class FarmlandWaterManager {
    ...
    public static <T extends SimpleTicket<Vec3>> T addCustomTicket(Level level, T ticket, ChunkPos masterChunk, ChunkPos... additionalChunks) {
        ...
        Map<ChunkPos, ChunkTicketManager<Vec3>> ticketMap = getOrCreateTicketMap(level);
    ...
    static void removeTickets(ChunkAccess chunk) {
        ...
        if (ticketManager != null) {
            ...
            for (SimpleTicket<Vec3> ticket : ticketManager.getTickets()) {
                if (ticket.unload(ticketManager)) { //remove if this is the master manager of the ticket
    ...
    private static Map<ChunkPos, ChunkTicketManager<Vec3>> getOrCreateTicketMap(LevelReader level) {
        synchronized (customWaterHandler) {
            return customWaterHandler.computeIfAbsent(level, id -> new MapMaker().weakValues().makeMap());
        }
    }

    private static ChunkTicketManager<Vec3> getTicketManager(ChunkPos pos, LevelReader level) {
        Preconditions.checkArgument(!level.isClientSide(), "Water region is only determined server-side");
        Map<ChunkPos, ChunkTicketManager<Vec3>> ticketMap;
        synchronized (customWaterHandler) {
            ticketMap = customWaterHandler.get(level);
        }
        if (ticketMap == null) {
            return null;
        }
