...
public class ForcedChunkManager {
    ...
    static <T extends Comparable<? super T>> boolean forceChunk(ServerLevel level, Identifier id, T owner, int chunkX, int chunkZ, boolean add,
        ...
        if (RegionizedWorldGuard.scheduleGlobalIfRegionized(
                level,
                "NeoForge forced chunk update",
                () -> forceChunk(level, id, owner, chunkX, chunkZ, add, forceNaturalSpawning, ticketGetter))) {
            return false;
        }
    ...
    public static class TicketTracker<T extends Comparable<? super T>> {
        ...
        public synchronized void deactivateTicketsOnClosing() {
        ...
        private synchronized void inheritDeactivated(TicketOwner<T> owner, TicketSet ticketSet) {
        ...
        private synchronized void activateAllDeactivatedSources() {
         ...
         */
        public synchronized boolean hasNoDeactivatedTickets() {
         ...
         */
        public synchronized boolean remove(TicketOwner<T> owner, long chunk, boolean forceNaturalSpawning, boolean targetDeactivated) {
         ...
         * @return {@code true} if the state changed.
         */
        private synchronized boolean add(TicketOwner<T> owner, long chunk, boolean forceNaturalSpawning) {
